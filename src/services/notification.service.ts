...
// Runs right after the action that queued the intent (after the response is
// sent) and again from the daily cron, which picks up anything left behind.
// claim_notification_intents leases rows, so overlapping runs never double-send.
...
export type DeliveryResult = { claimed: number; sent: number; failed: number };

export async function deliverPendingNotifications(limit = 50): Promise<DeliveryResult> {
  const db = createServiceClient();
  const { data, error } = await db.rpc("claim_notification_intents", { p_limit: limit, p_lease_seconds: 120 });
  if (error) throw new Error(`claim_failed:${error.code}`);
...
/** Call after an action that may have queued intents. Never blocks or fails the action. */
export function deliverNotificationsSoon() {
  after(async () => {
    try {
      await deliverPendingNotifications(20);
    } catch (e) {
      logger.warn("notifications.deliver_after_failed", { error: e });
    }
  });
}
